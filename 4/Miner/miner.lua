...
function table_to_string(table, indent)
    ...
    for key, value in pairs(table) do
        if type(value) == "table" then
            string_table = string_table .. string.rep(" ", indent) .. key .. ":\n"
            string_table = string_table .. table_to_string(value, indent + 4)
        else
            string_table = string_table .. string.rep(" ", indent) .. key .. ": " .. value .. "\n"
        end
    end
    return string_table
end
...
function transmit(bot)
    local message = bot.name .. ":\n"
    message = message .. "\tStatus: " .. bot.status .. "\n"
    message = message .. "\tFuel level: " .. bot.fuel_level .. "\n"
    message = message .. "\tTrip distance: " .. bot.trip_distance .. "\n"
    message = message .. "\tPosition: " .. vector_to_string(bot.position) .. "\n"
    message = message .. "\tDirection: " .. vector_to_string(bot.vector) .. "\n"
    message = message .. "\tBranches dug: " .. bot.branches_dug .. "\n"
    peripheral
        .find("modem")
        .transmit(8, 1, message)
end

-- BOT CONSTRUCTOR --

local function bot_constructor(
    position, vector, storage, chunks, first_turn_direction, branch_cycles_to_dig
)
    storage = storage or { x = 0, y = 0, z = 0 }
    chunks = chunks or 12
...
local function main(t_args)
    ...
    print("Storage: " .. vector_to_string(bot.storage_location))
    
    local out_file = io.open("Miner/bot.txt", "w")
    out_file:write(table_to_string(bot))
    out_file:close()
    ...
    while (
        ...
        for i = 1, 2 do
            ...
            out_file = io.open("Miner/bot.txt", "a")
            out_file:write("\n" .. table_to_string(bot) .. "\n")
            out_file:close()
